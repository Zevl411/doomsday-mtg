...
describe('tournament Deck normalization', () => {
  ...
  it('reports unknown structured sections without parsing metadata as cards', () => {
    ...
    expect(result.cards).toEqual([expect.objectContaining({ name: 'Island', board: 'mainboard' })]);
    expect(result.cards.map((card) => card.name)).not.toContain('game');
    expect(result.issues).toEqual([expect.objectContaining({ code: 'unsupported_board' })]);
  });
  ...
  it('combines duplicate normalized names and reports the issue', () => {
    ...
    expect(result.issues).toEqual(
      expect.arrayContaining([expect.objectContaining({ code: 'duplicate_normalized_card' })]),
    );
