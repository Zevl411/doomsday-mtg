...
import { normalizePreferences, saveUserPreferences } from './userPreferencesRepository';
...
describe('user preference response validation', () => {
  ...
  it('retries without price currency during a rolling schema deployment', async () => {
    ...
    await expect(saveUserPreferences(DEFAULT_USER_PREFERENCES, 'user-one')).resolves.toBe(true);

    expect(mocks.upsert).toHaveBeenCalledTimes(2);
    expect(mocks.upsert.mock.calls[0]?.[0]).toHaveProperty('price_currency', 'USD');
    expect(mocks.upsert.mock.calls[1]?.[0]).not.toHaveProperty('price_currency');
  });
});
