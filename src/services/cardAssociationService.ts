...
export const cardAssociationService = {
  analyze(
    ...
    const significant = associations.filter((association) =>
      isStatisticallySignificant(association, thresholds),
    );
    const maximumRawScore = Math.max(0, ...significant.map(rawAssociationScore));

    return significant.map((association) => ({
      ...association,
      strength: associationStrength(association),
      normalizedScore:
        maximumRawScore === 0 ? 0 : rawAssociationScore(association) / maximumRawScore,
...
export function isStatisticallySignificant(
  ...
  return (
    association.sampleSize >= thresholds.minimumSampleSize &&
...
export function associationStrength(association: CardAssociation): AssociationStrength {
  if (association.lift >= 1.5 && association.confidence >= 0.5 && association.deckCount >= 10)
    return 'strong';
  if (association.lift >= 1.2 && association.confidence >= 0.25 && association.deckCount >= 5)
    return 'moderate';
  return 'emerging';
}
...
function rawAssociationScore(association: CardAssociation): number {
  ...
  return association.support * association.confidence * boundedLift * reliability;
}
