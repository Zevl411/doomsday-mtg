...
describe('card association repository response validation', () => {
  ...
  it.each([
    ...
    expect(() => parseCardAssociationRows(value)).toThrow(
      'Card association response was invalid.',
    )
  })

  it('deduplicates validated cluster identities', () => {
    expect(parseCardAssociationClusterRows([{
      cluster_id: oracleA,
      member_oracle_ids: [oracleA, oracleB, oracleB, 
  ...
  it('maps database failures to a stable public error', async () => {
    ...
    await expect(
      cardAssociationRepository.getAssociations('kinnan', oracleA),
    ).rejects.toThrow('Unable to load observed card associations.')
  })
})
