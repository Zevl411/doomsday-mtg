...
const UNTRACKED_BOARDS = new Set<DeckBoard>(['companion', 'acquireboard', 'tokens']);
...
export function parseDecklist(text: string, selectedFormat?: DecklistFormat): ParsedDecklist {
  ...
  originalLines.forEach((originalLine, index) => {
    ...
    if (lineSection === 'mainboard' && isLikelyCategoryHeading(cardInput, originalLines, index)) {
    ...
    if (UNTRACKED_BOARDS.has(lineSection)) {
      const ignoredLine = parseIgnoredLine(cardInput, lineNumber, lineSection, originalLine);
      ignoredCounts.set(lineSection, (ignoredCounts.get(lineSection) ?? 0) + ignoredLine.quantity);
      return;
    }

    const parsedLine = parseCardLine(cardInput, lineNumber, lineSection, originalLine);
...
function getInlineBoardPrefix(input: string): { board: DeckBoard; cardText: string } | null {
  ...
  return board ? { board, cardText: (match[2] ?? '').trim() } : null;
}
...
function parseCardLine(
  ...
  if (/^[+-]?\d+(?:\.\d+)?x?(?:\s|$)/i.test(withoutComment) || /^\d+x\S/i.test(withoutComment)) {
    return createQuantityIssue(lineNumber, originalText);
  }
...
function createQuantityIssue(lineNumber: number, input: string): DeckImportIssue {
  return {
    lineNumber,
    input,
...
function isLikelyCategoryHeading(input: string, lines: string[], index: number): boolean {
