<template>
  <v-container class="pa-0" fluid>
    ...
    <p class="mb-6 text-medium-emphasis">cEDH results with links to their original source.</p>
    <v-card class="mb-5 pa-3" border>
      <AppMobileFilterPanel>
        <v-row align="center" density="comfortable">
          ...
          <v-col class="d-flex align-center ga-2" cols="12" sm="6" lg="3">
    ...
    <AppLoadingSkeleton v-if="loading" :count="8" label="Loading tournaments" variant="table" />
...
const sizeRange = ref<TournamentSizeRange>(savedFilters?.sizeRange ?? 'standard');
const timePeriod = ref<TournamentTimePeriod>(savedFilters?.timePeriod ?? 'all');
const sortOrder = ref<TournamentSortOrder>(savedFilters?.sortOrder ?? 'date-desc');
// The default keeps unresolved tournament shells out of the primary list.
const registeredCommandersOnly = ref(savedFilters?.registeredCommandersOnly ?? true);
...
watch([sizeRange, timePeriod, sortOrder, registeredCommandersOnly], () => {
...
async function load() {
  ...
  try {
    tournaments.value = await tournamentRepository.getRecentTournaments({
      ...
      tournamentSort: sortOrder.value.startsWith('size') ? 'player-count' : 'date',
