...
describe('DeckLibraryCard', () => {
  it('shows commander and quantity-based board counts', () => {
    ...
    expect(wrapper.find('button[aria-label="Open deck actions"]').exists()).toBe(true);
    expect(
      wrapper.get('.deck-card-content-header').findAll('.deck-card-menu-icon circle'),
    ).toHaveLength(3);
  ...
  it('hides owner-only actions when displaying a public deck', () => {
    ...
    expect(wrapper.find('button[aria-label="Compare deck"]').exists()).toBe(false);
    expect(wrapper.find('button[aria-label="Rename deck"]').exists()).toBe(false);
    expect(wrapper.find('button[aria-label="Duplicate deck"]').exists()).toBe(false);
    expect(wrapper.find('button[aria-label="Delete deck"]').exists()).toBe(false);
  });
  ...
  it('opens the deck when its Commander artwork is selected', async () => {
    ...
    const artwork = wrapper.get('button[aria-label="Open Artwork link"]');
  ...
  it('blends both Commander images for a partner deck', () => {
    ...
    expect(artworkLayers[0]?.attributes('class')).toContain('deck-summary-art-layer--primary');
    expect(artworkLayers[1]?.attributes('class')).toContain('deck-summary-art-layer--partner');
  });
});
