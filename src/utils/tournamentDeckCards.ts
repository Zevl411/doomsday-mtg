...
export function groupTournamentDeckCards(cards: TournamentDeckCard[]): TournamentDeckCard[] {
...
export function applyScryfallCardDetails(
  ...
  return cards.map((card) => {
    ...
    return {
      ...card,
      typeLine: resolved?.card_faces?.[0]?.type_line ?? resolved?.type_line ?? card.typeLine,
...
function addCardDetails(details: Map<string, ScryfallCard>, name: string, card: ScryfallCard) {
...
export function hasExpectedTournamentDeckCards(
  ...
  const mainboardCount = cards.reduce((total, card) => total + card.quantity, 0);
  return (
    commanders.length > 0 &&
    (expectedMainboardCount === undefined || mainboardCount >= expectedMainboardCount)
  );
}
...
function addExportSection(sections: string[], heading: string, cards: ExportableTournamentCard[]) {
