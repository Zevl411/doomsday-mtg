...
const { getSession, onAuthStateChange, signInWithOtp, signInWithPassword, signUp } = vi.hoisted(
  () => ({
    getSession: vi.fn(),
...
describe('authentication store', () => {
  ...
  it('waits for email confirmation instead of signing in new registrations', async () => {
    ...
    await expect(auth.register('new-player@example.com', 'password')).resolves.toBe(true);
