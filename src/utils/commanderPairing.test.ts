...
describe('commander pairing', () => {
  it('allows two commanders with standalone Partner', () => {
    expect(
      validateCommanderPairing(card('Tymna', 'Partner'), card('Kraum', 'Partner')).allowed,
    ).toBe(true);
  });
  ...
  it('supports Background and Doctor pairings', () => {
    ...
    const background = card('Noble Heritage', '', 'Legendary Enchantment — Background');
  ...
  it('creates targeted filters for every supported pairing mechanic', () => {
    expect(getPartnerSearchFilter(card('Tymna', 'Partner'))).toBe('o:/^Partner( \\\\(|$)/');
    expect(getPartnerSearchFilter(card('Cecily', 'Partner—Friends forever'))).toBe(
      'o:"Partner—Friends forever"',
    );
    expect(getPartnerSearchFilter(card('Volo', 'Choose a Background'))).toBe('t:background');
    expect(getPartnerSearchFilter(card('Rose Tyler', "Doctor's companion"))).toBe('t:doctor');
  });
});
