...
export function extractCommanders(decklist: unknown, deckObject: unknown): CommanderExtraction {
  ...
  for (const line of lines) {
    ...
    if (inCommanderSection && heading && /^[a-z][\w /'-]+$/i.test(heading) && !/^\d/.test(clean)) {
      break;
    }
