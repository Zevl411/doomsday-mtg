  ...
  <v-container class="pa-0" fluid>
    <div v-if="!embedded" class="mb-5">
      ...
      <p class="text-medium-emphasis">
        Explore observed co-occurrence in complete tournament Decks. These relationships are not
        causal and are not recommendations.
      </p>
    </div>
      ...
      <AppMobileFilterPanel>
        ...
        <div class="association-filter-grid mt-2">
          ...
          <div class="association-actions d-flex align-center ga-2">
            <v-btn density="comfortable" variant="text" @click="resetFilters"> Reset </v-btn>
            <v-btn
              color="primary"
              density="comfortable"
              :disabled="!canLoad"
    ...
    <v-card border class="metric-key mb-3 pa-3">
      ...
      <div>
        <strong>Confidence</strong> — Percent of Decks with the searched card that also contain this
        card.
      </div>
      <div>
        <strong>Lift</strong> — Co-occurrence compared with the card's usual inclusion rate; 1×
        means no observed increase.
      </div>
      <div>
        <strong>Sample</strong> — Complete tournament Decks included after applying the filters.
      </div>
    </v-card>
    ...
    <v-card v-else-if="hasSearched && !associations.length" border class="pa-8 text-center">
      No statistically reliable associations match these filters. Widen the sample or choose another
      card.
    </v-card>
    <v-card v-else-if="associations.length" border>
      <v-card-title> Strongest observed associations for {{ selectedCard?.name }} </v-card-title>
        ...
        >
          ...
          <v-card-text class="pa-3">
            ...
            <div class="text-caption text-medium-emphasis mt-2">
              {{ association.deckCount }} joint Decks · {{ association.strength }} observed
              association
            </div>
          </v-card-text>
        </v-card>
...
const minimumConfidencePercent = ref(DEFAULT_ASSOCIATION_THRESHOLDS.minimumConfidence * 100);
...
const canLoad = computed(() => Boolean(commanderKey.value && selectedCard.value?.oracle_id));
...
async function loadAssociations() {
  ...
  try {
    ...
    const rows = await cardAssociationRepository.getAssociations(commanderKey.value, oracleId, {
    ...
    associations.value = [];
    errorMessage.value =
      error instanceof Error ? error.message : 'Unable to load card associations.';
  } finally {
    loading.value = false;
  }
}
...
function resetFilters() {
  ...
  minimumConfidencePercent.value = DEFAULT_ASSOCIATION_THRESHOLDS.minimumConfidence * 100;
