-- Exclude cards already in the personal Deck before candidate ranking.
-- Previously the RPC ranked owned staples first, limited the candidate set,
-- and only then removed those cards. Full Deck payloads could therefore
-- return no rows and performed unnecessary source-to-owned-card joins.
...
as $$
  ...
  joint as (
    ...
    join deck_cards candidate
     ...
     and candidate.canonical_card_id <> source.source_card_id
    join public.canonical_cards candidate_identity
      on candidate_identity.id = candidate.canonical_card_id
     and not (
       candidate_identity.oracle_id = any(
         coalesce(p_source_oracle_ids, '{}'::uuid[])
       )
     )
