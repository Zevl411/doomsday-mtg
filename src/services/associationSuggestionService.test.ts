...
function row(values: Partial<AssociationSuggestionRow> = {}): AssociationSuggestionRow {
  return {
    commanderKey: 'kinnan',
...
describe('association-based suggestion evidence aggregation', () => {
  it('requires the configured number of distinct supporting cards', () => {
    expect(
      associationSuggestionService.buildSuggestions([row()], [], { minimumSupportingCards: 2 }),
    ).toEqual([]);

    expect(
      associationSuggestionService.buildSuggestions(
        [
          row(),
          row({
