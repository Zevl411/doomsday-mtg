<template>
  <v-container class="tournament-deck-view pa-0" fluid>
    <AppLoadingSkeleton v-if="loading" label="Loading tournament Deck" variant="detail" />
    ...
    <template v-else-if="deck">
        ...
        <div class="deck-hero__content">
          <div>
            <div class="d-flex align-center flex-wrap ga-3">
              ...
              <ColorIdentitySymbols :colors="deck.entry.colorIdentity" size="small" />
              <v-chip
            ...
            <p class="text-body-1 mt-1">
              ...
              · Place {{ deck.entry.standing ?? '—' }} · {{ recordLabel }}
            </p>
            <p class="text-medium-emphasis">
              {{ deck.tournament.name }} · {{ formattedEventDate }} ·
      ...
      <div class="deck-overview mb-4">
        <v-card border class="commander-showcase" rounded="lg" variant="flat">
        ...
        <v-card border rounded="lg" variant="flat">
          ...
          <v-card-text class="pa-4">
            ...
            <div class="mana-curve">
              <div v-for="bucket in manaCurve" :key="bucket.label" class="mana-curve__column">
        ...
        <v-card border class="card-preview" rounded="lg" variant="flat">
          <v-card-title class="widget-header-bar">Card Preview</v-card-title>
          <v-card-text class="pa-4">
            <TournamentCardImage v-if="previewCard" :card="previewCard" />
      ...
      <v-card v-if="comparison" border class="mb-4" rounded="lg" variant="flat">
        <v-card-title class="widget-header-bar"> Commander Metagame Context </v-card-title>
        <v-card-text class="d-flex flex-wrap align-center ga-3 pa-4">
          <v-chip color="primary" variant="tonal">
            {{ comparison.sharedCardCount }} commonly played cards
          </v-chip>
          <v-chip variant="tonal"> {{ percent(comparison.similarity) }} aggregate overlap </v-chip>
          <v-chip color="warning" variant="tonal">
            {{ comparison.missingCoreCards.length }} absent core cards
          </v-chip>
          <v-chip variant="tonal"> {{ comparison.rareCards.length }} uncommon inclusions </v-chip>
          <span class="text-caption text-medium-emphasis">
      ...
      >
        <v-card-title class="board-header widget-header-bar">
          <span class="text-capitalize"> {{ board }} ({{ boardQuantity(board) }}) </span>
      ...
      <p class="text-caption">
        ...
        <a :href="sourceAttribution(deck.source).url" rel="noopener noreferrer" target="_blank">
          {{ deck.source === 'topdeck' ? 'TopDeck.gg' : 'EDHTop16' }} </a
...
import { getSortableCardType, type TournamentDeckSort } from '../utils/tournamentDeckCards';
...
const populatedBoards = computed(() =>
  boardOrder.filter((board) => deck.value?.cards.some((card) => card.board === board)),
);
...
const commanderCards = computed(() => displayCardsForBoard('commander'));
...
const averageManaValue = computed(() => {
  const cards = mainboardCards.value.filter((card) => !/\bLand\b/i.test(card.typeLine ?? ''));
  ...
  return labels
    .map((label) => ({
      ...
      count: mainboardCards.value.reduce(
        (sum, card) =>
          sum + (new RegExp(`\\b${label}\\b`, 'i').test(card.typeLine ?? '') ? card.quantity : 0),
        0,
      ),
    }))
...
const formattedEventDate = computed(() => formatDate(deck.value?.tournament.date));
const formattedImportDate = computed(() => formatDate(deck.value?.importedAt));
...
onMounted(async () => {
  try {
    ...
    previewCard.value = commanderCards.value[0] ?? displayCardsForBoard('mainboard')[0] ?? null;
  } catch (error) {
...
function cardsForBoard(board: TournamentDeckBoard) {
  ...
  return [...cards].sort((left, right) => {
    if (decklistSort.value === 'mana-value') {
      const difference =
        (left.manaValue ?? Number.MAX_SAFE_INTEGER) - (right.manaValue ?? Number.MAX_SAFE_INTEGER);
...
function toDisplayCard(card: NormalizedTournamentDeckCard): TournamentDeckCard {
  ...
  return {
    ...
    imageUrl: resolved?.image_uris?.normal ?? resolved?.card_faces?.[0]?.image_uris?.normal ?? '',
...
async function copyToMyDecks() {
  ...
  if (!cards.length) {
    try {
      cards = await getCardsByExactNames(deck.value.cards.map((card) => card.cardName));
    } catch (error) {
      console.warn('Unable to hydrate copied tournament card images.', error);
    }
  }
  const cardLookup = createTournamentCardLookup(cards);
  const copy = createEmptyDeck(`${deck.value.commanderName} — ${deck.value.tournament.name}`);
