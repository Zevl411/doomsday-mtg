...
export function displayRegionKey(regionKey: string) {
  ...
  return regionKey.replace('country:', '').replace('/state:', ' · ');
}
...
export function displayTournamentLocation(tournament: Tournament) {
  ...
  const locality = [tournament.city, tournament.stateRegion].filter(Boolean).join(', ');
  const parts = [tournament.venueName, locality, tournament.countryCode].filter(Boolean);
