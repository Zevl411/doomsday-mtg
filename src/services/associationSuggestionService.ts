...
export const associationSuggestionService = {
  buildSuggestions(
    ...
    const minimumSupportingCards = Math.max(
      1,
      filters.minimumSupportingCards ?? 2,
    )
    ...
    for (const row of rows) {
      ...
      if (existingIndex === -1) candidateRows.push(row)
        ...
        evidenceScore(row) > evidenceScore(candidateRows[existingIndex]!)
    ...
    const unnormalized = [...grouped.values()]
      ...
      .map((evidence) => {
        ...
        return {
          ...
          aggregateScore: evidence.reduce(
            (total, item) => total + evidenceScore(item),
            0,
          ),
    ...
    return unnormalized
      .slice(0, Math.max(1, filters.limit ?? 30))
      .map((suggestion) => ({
        ...suggestion,
        aggregateScore: maximumScore === 0
          ? 0
          : suggestion.aggregateScore / maximumScore,
      }))
  },
  ...
  attachCards(
    ...
    const byOracleId = new Map(
      cards
        .filter((card): card is ScryfallCard & { oracle_id: string } =>
    ...
    return suggestions.map((suggestion) => ({
      ...suggestion,
      suggestedCard:
        byOracleId.get(suggestion.suggestedOracleId.toLowerCase()) ?? null,
    }))
  },
}
