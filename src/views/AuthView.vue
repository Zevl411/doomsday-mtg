      ...
      <v-card border color="surface" rounded="lg">
        ...
        <v-card-text class="px-6">
          <p class="mb-4 text-medium-emphasis">
            Sign in to sync decks across devices. Guest deck building remains available without an
            account.
          </p>
          ...
          <v-alert v-if="auth.registrationConfirmationRequired" type="success" variant="tonal">
        ...
        <v-card-actions class="px-6 pb-6">
          ...
          <v-spacer />
          <v-btn :loading="auth.loading" variant="outlined" @click="submit(true)"> Register </v-btn>
          <v-btn :loading="auth.loading" variant="text" @click="sendMagicLink"> Email Link </v-btn>
          <v-btn color="primary" :loading="auth.loading" @click="submit(false)"> Sign In </v-btn>
        </v-card-actions>
      </v-card>
    </v-col>
  </v-row>
...
async function sendMagicLink() {
  magicLinkSent.value = await auth.sendMagicLink(email.value.trim());
}
</script>
