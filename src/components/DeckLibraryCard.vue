<template>
  ...
  >
    <button
      ...
      :class="['deck-selection-control', { 'deck-selection-control--selected': selected }]"
      type="button"
      @click.stop="toggleSelection"
    >
      <svg v-if="selected" aria-hidden="true" class="deck-selection-check" viewBox="0 0 24 24">
    ...
    <v-card-item class="deck-card-content-header">
      <div class="deck-card-header-controls">
        ...
        <v-menu v-model="showMobileActions" location="bottom end" :close-on-content-click="false">
          <template #activator="{ props: activatorProps }">
            ...
            >
              <svg aria-hidden="true" class="deck-card-menu-icon" viewBox="0 0 24 24">
          ...
          <v-list density="comfortable">
            ...
            <v-list-item title="Duplicate" @click="emitMobileAction('duplicate', deck.id)">
              <template #prepend>
                <DeckActionIcon compact name="duplicate" />
              </template>
            </v-list-item>
            <v-list-item title="Delete" @click="emitMobileAction('delete', deck.id)">
              <template #prepend>
      ...
      <v-card-title>
        <button class="deck-card-title-button" type="button" @click="openDeck">
          {{ deck.name }}
        </button>
      </v-card-title>
      <v-card-subtitle>
    ...
    <v-card-text class="flex-grow-1">
      <div class="d-flex flex-wrap ga-2">
        <v-chip size="x-small" variant="tonal"> Main {{ boardCount(deck.cards) }} </v-chip>
        <v-chip size="x-small" variant="tonal"> Side {{ boardCount(deck.sideboard) }} </v-chip>
      ...
      <div class="text-caption text-medium-emphasis">Updated {{ updatedLabel }}</div>
    </v-card-text>
    ...
    >
      ...
      <v-spacer />
      <v-btn-group class="deck-secondary-actions" density="comfortable" divided variant="text">
        <v-btn
        ...
        <v-btn aria-label="Rename deck" icon title="Rename deck" @click="emit('rename', deck.id)">
          <DeckActionIcon compact name="rename" />
        </v-btn>
        <v-btn
...
const commanderImage = computed(() =>
  props.deck.commander ? getCardArt(props.deck.commander) : undefined,
);
const partnerImage = computed(() =>
  props.deck.partnerCommander ? getCardArt(props.deck.partnerCommander) : undefined,
);
const totalCount = computed(() => getTotalDeckCardCount(props.deck));
const deckVisibility = computed<DeckVisibility>(() => props.deck.visibility ?? 'private');
...
function emitMobileAction(action: 'compare' | 'rename' | 'duplicate' | 'delete', deckId: string) {
