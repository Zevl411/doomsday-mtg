import type { LocationPrecision, NormalizedTournamentLocation } from './types.ts';
...
export function normalizeTournamentLocation(
  ...
  const locationText = [venueName, city, stateRegion, countryCode, clean(input?.address)]
    .filter(Boolean)
    .join(' ');
  ...
  return {
    ...
    locationConfidence:
      locationPrecision === 'unknown' ? null : locationPrecision === 'exact' ? 'high' : 'medium',
  };
}
...
export function createRegionKey(
  ...
  const state = stateRegion
    ...
    .replace(/^-|-$/g, '')
    .toUpperCase();
  return state ? `country:${countryCode}/state:${state}` : `country:${countryCode}`;
}
