...
export function getCommanderColorIdentity(deck: Deck): string[] {
  return [...new Set(getDeckCommanders(deck).flatMap((card) => card.color_identity))];
}
...
export function validateCommanderPairing(
  ...
  if (namesMatch(firstNamedPartner, second.name) && namesMatch(secondNamedPartner, first.name)) {
    return { allowed: true };
  }
