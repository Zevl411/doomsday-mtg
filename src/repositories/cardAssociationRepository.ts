...
export const cardAssociationRepository = {
  async getAssociations(
    ...
    requireSupabase();
    const { data, error } = await supabase!.rpc('get_commander_card_associations', {
      p_commander_key: commanderKey,
      p_source_oracle_id: sourceOracleId,
      p_start_date: filters.startDate ?? null,
      p_end_date: filters.endDate ?? null,
      p_region_key: null,
      p_minimum_tournament_size: nonNegativeInteger(filters.minimumTournamentSize, 0),
      p_maximum_standing: optionalPositiveInteger(filters.maximumStanding),
      p_minimum_sample_size: positiveInteger(filters.minimumSampleSize, 20),
      p_minimum_occurrence_count: positiveInteger(filters.minimumOccurrenceCount, 3),
      p_minimum_confidence: nonNegativeNumber(filters.minimumConfidence, 0),
  ...
  async getClusters(
    ...
    requireSupabase();
    const { data, error } = await supabase!.rpc('get_commander_card_association_clusters', {
      p_commander_key: commanderKey,
      p_minimum_deck_count: positiveInteger(filters.minimumDeckCount, 5),
      p_minimum_confidence: nonNegativeNumber(filters.minimumConfidence, 0.2),
...
export function parseCardAssociationClusterRows(value: unknown): CardAssociationCluster[] {
