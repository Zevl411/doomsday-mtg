...
export class TopDeckProvider implements TournamentProvider {
  ...
  async listTournaments(options: ProviderListOptions): Promise<ProviderTournament[]> {
    ...
    return Promise.all(
      tournaments.map(async (raw) => {
        ...
        if (options.enrichLocation && location.locationPrecision === 'unknown') {
        ...
        return {
          ...
          name: readString(tournament, ['tournamentName', 'name']) ?? `TopDeck tournament ${id}`,
          date: readDate(tournament.startDate ?? tournament.date),
          playerCount:
            readNumber(tournament, ['participantCount', 'playerCount']) ?? standings.length,
          url:
            readString(tournament, ['url', 'sourceUrl']) ??
            `https://topdeck.gg/event/${encodeURIComponent(id)}`,
          sourceUpdatedAt: readDate(tournament.updatedAt ?? tournament.lastUpdated) ?? undefined,
          location,
          raw,
        };
      }),
  ...
  async listEntries(tournament: ProviderTournament): Promise<ProviderTournamentEntry[]> {
    const standings =
      this.cachedStandings.get(tournament.sourceTournamentId) ??
  ...
  private async request(path: string, init: RequestInit = {}) {
    ...
    const baseUrl = (this.options.baseUrl ?? 'https://topdeck.gg/api').replace(/\/$/, '');
...
function standingColumns() {
  return ['name', 'id', 'decklist', 'wins', 'draws', 'losses', 'winRate'];
}
...
function mapStanding(raw: unknown, index: number): ProviderTournamentEntry {
  ...
  return {
    sourceEntryId: readString(standing, ['id', 'standingId']) ?? `standing-${index + 1}`,
    ...
    decklistUrl:
      readString(standing, ['decklistUrl', 'decklist_url']) ??
      (typeof decklist === 'string' && /^https?:\/\//.test(decklist) ? decklist : undefined),
