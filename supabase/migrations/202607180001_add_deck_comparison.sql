-- Public comparison receives only analytical card keys from the browser. It
-- never reads public.decks, user IDs, or private provider payloads.
  ...
  counts as (
    select
      ...
      (
        select count(*)::bigint
        from (
          select identity_key from deck_cards x where x.tournament_deck_id = s.id
          union
