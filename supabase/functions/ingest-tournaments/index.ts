...
import type { ProviderTournament, ProviderTournamentEntry, TournamentProvider } from './types.ts';
...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const excludedTournaments = options.excludeCasualEvents
      ? fetchedTournaments.filter(
          (tournament) => !evaluateTournamentTitle(tournament.name, excludedKeywords).included,
        )
      : [];
    const excludedIds = new Set(
      excludedTournaments.map((tournament) => tournament.sourceTournamentId),
    );
    const tournaments = fetchedTournaments.filter(
      (tournament) => !excludedIds.has(tournament.sourceTournamentId),
    );
    ...
    if (!fetchedTournaments.length) {
      report.validationIssues.push(createEmptyResultMessage(options));
    }
    ...
    await mapWithConcurrency(tournaments, 3, async (tournament) => {
      try {
        ...
        const { data: explicitMatch } = await admin
          ...
          .limit(1)
          .maybeSingle();
        const { error: linkError } = await admin.from('tournament_source_links').upsert(
          {
        ...
        if (normalizedEntries.length) {
          const entryKeys = normalizedEntries.map((entry) => entry.source_entry_key);
...
function getProvider(source: 'edhtop16' | 'topdeck'): TournamentProvider {
  ...
  if (source === 'edhtop16') {
    return new EdhTop16Provider(Deno.env.get('EDHTOP16_BASE_URL') ?? 'https://edhtop16.com');
  }
...
function validateRequest(
  value: unknown,
): Required<Pick<IngestionRequest, 'provider' | 'minimumPlayers' | 'dryRun'>> &
  Omit<IngestionRequest, 'provider' | 'minimumPlayers' | 'dryRun'> {
  if (!isRecord(value) || (value.provider !== 'edhtop16' && value.provider !== 'topdeck')) {
  ...
  const maximumPlayers =
    value.maximumPlayers === undefined || value.maximumPlayers === ''
      ? undefined
  ...
  const last = value.last === undefined || value.last === '' ? undefined : Number(value.last);
  ...
  const tournamentIds = Array.isArray(value.tournamentIds)
    ? value.tournamentIds.map((id) => id.trim()).filter(Boolean)
    : [];
  return {
...
function mapTournament(tournament: ProviderTournament, provider: TournamentProvider) {
  return {
    source: provider.source,
    source_tournament_id: tournament.sourceTournamentId,
...
async function mapEntry(entry: ProviderTournamentEntry, tournamentId: string) {
...
function createEmptyResultMessage(options: {
  ...
  const ids = options.tournamentIds?.length
    ...
    : '';
  const maximum =
    options.maximumPlayers === undefined ? '' : ` and at most ${options.maximumPlayers}`;
...
async function handleJobAction(admin: AdminClient, request: JobActionRequest, userId: string) {
  if (request.action === 'clear-tournament-data') {
    ...
    const { data, error } = await admin.rpc('clear_tournament_ingestion_data');
    if (error) throw error;
    return data;
  }
  ...
  if (request.action === 'create-job' || request.action === 'create-deck-job') {
    if (request.provider !== 'topdeck' && request.provider !== 'edhtop16')
    ...
    const { data: job, error } = await admin
      ...
      .single();
    if (error) throw error;
    const { error: batchError } = await admin.rpc('create_tournament_ingestion_batches', {
...
async function purgeCasualEvents(admin: AdminClient, request: JobActionRequest) {
  ...
  const matches = (data ?? []).filter(
    (tournament) => !evaluateTournamentTitle(tournament.name, keywords).included,
  );
  const entriesAffected = matches.reduce(
    (total, tournament) => total + Number(tournament.tournament_entries?.[0]?.count ?? 0),
    0,
  );
  let eventsPurged = 0;
