...
const IGNORED_STRUCTURED_SECTIONS = new Set(['metadata', 'stickers', 'attractions', 'tokens']);
...
export function normalizeStructuredDeck(value: unknown): CandidateResult {
  if (!isRecord(value)) {
    return {
      cards: [],
      issues: [{ code: 'invalid', message: 'Structured deck data is invalid.' }],
    };
  }
...
export function normalizePlaintextDeck(text: string): CandidateResult {
  ...
  for (const original of text.split(/\r?\n/)) {
    ...
    const inlineBoard = inline ? BOARD_NAMES[normalizeHeading(inline[1] ?? '')] : undefined;
...
export function evaluateDeckCompleteness(
  ...
  if (commanderCount > 2) {
    issues.push({
      code: 'unsupported_commanders',
      message: 'More than two Commanders were found.',
    });
  }
  ...
  return Object.entries(value).flatMap(([name, details]) => {
    const quantity =
      typeof details === 'number'
        ? details
        : isRecord(details)
          ? Number(details.quantity ?? details.count ?? 1)
          : 1;
