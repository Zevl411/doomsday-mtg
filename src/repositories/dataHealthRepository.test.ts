...
describe('data health response validation', () => {
  ...
  it('rejects missing, malformed, and extra aggregate rows', () => {
    expect(() => parseDataHealthSummary([])).toThrow('The data health response was invalid.');
  ...
  it('validates readiness rows and preserves paired diagnostics', () => {
    ...
    expect(() => parseCommanderReadinessRows([{ ...row, sample_status: 'great' }])).toThrow(
      'The Commander readiness response was invalid.',
    );
  });
  ...
  it('validates unresolved-card provider counts and ingestion jobs', () => {
    ...
    expect(parseUnresolvedCardRows([unresolved])[0]?.providerBreakdown).toEqual({});
    expect(() =>
      parseUnresolvedCardRows([
        {
          ...unresolved,
    ...
    expect(() => parseIngestionJobHealthRows([{ ...job, attempts: '2' }])).toThrow(
      'The ingestion job health response was invalid.',
    );
  });
  ...
  it('validates bounded regional coverage rows', () => {
    ...
    expect(() => parseRegionCoverageRows([{ region_key: 'country:US' }])).toThrow(
      'The region coverage response was invalid.',
    );
  });
});
