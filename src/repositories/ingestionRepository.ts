...
export type IngestionJobStatus =
  'pending' | 'running' | 'completed' | 'completed_with_errors' | 'paused' | 'cancelled';
...
export const ingestionRepository = {
  ...
  async ingest(options: IngestionOptions): Promise<IngestionReport> {
    ...
    const { data, error } = await supabase.functions.invoke('ingest-tournaments', {
      body: options,
   ...
   */
  async ingestAllTournaments(options: IngestionOptions): Promise<IngestionReport> {
    const windows = createDateWindows(options.startDate, options.endDate, 3);
    if (windows.length <= 1 || options.last || options.tournamentIds?.length) {
    ...
    for (const window of windows) {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Unknown tournament metadata ingestion error.';
   ...
   */
  async createDeckNormalizationJob(options: CreateIngestionJobOptions): Promise<string> {
    const result = await invokeAuthenticatedFunction({
  ...
  async purgeCasualEvents(options: PurgeCasualEventsOptions): Promise<PurgeCasualEventsReport> {
    const result = await invokeAuthenticatedFunction({
    ...
    return {
      ...
      titles: result.titles.filter((title): title is string => typeof title === 'string'),
  ...
  async ingestTournamentDecks(
    ...
    const { data, error } = await supabase.functions.invoke('ingest-tournament-decks', {
      body: options,
  ...
  async ingestAllTournamentDecks(
    ...
    for (let batch = 0; batch < 100; batch += 1) {
      ...
      try {
        ...
        const message =
          error instanceof Error ? error.message : 'Unknown card-level ingestion error.';
  ...
  async getJobs(): Promise<IngestionJob[]> {
    ...
    const { data, error } = await supabase
      ...
      .select('*, tournament_ingestion_batches(last_error, updated_at)')
  ...
  async getDashboardMetrics(): Promise<IngestionDashboardMetrics> {
    ...
    const [
      ...
      supabase
        .from('tournaments')
        .select('id, name, source, event_date, player_count, imported_at')
      ...
      supabase.from('possible_tournament_matches').select('*', { count: 'exact', head: true }),
      supabase
        .from('linked_tournament_events')
        .select('tournament_id', { count: 'exact', head: true }),
      supabase.from('tournament_decks').select('*', { count: 'exact', head: true }),
      supabase
    ...
    const recentTournaments = ((recentResult.data ?? []) as AdminTournamentRow[]).map((row) => ({
      id: row.id,
      name: row.name,
      source: row.source,
    ...
    return {
      ...
      unknownLocationCount: (tournamentsResult.count ?? 0) - (locationResult.count ?? 0),
      ...
      deckCoverage: ((coverageResult.data ?? []) as DeckCoverageRow[]).map((row) => ({
...
async function invokeAuthenticatedFunction(body: Record<string, unknown>): Promise<unknown> {
  if (!supabase) throw new Error('Supabase is not configured.');
  const { data: sessionData, error: sessionError } = await supabase.auth.getSession();
  const accessToken = sessionData.session?.access_token;
  if (sessionError || !accessToken) {
...
export function createAuthorizationHeaders(accessToken: string): Record<string, string> {
...
export function parseTournamentDeckIngestionReport(value: unknown): TournamentDeckIngestionReport {
  if (
    !isRecord(value) ||
