...
describe('tournament development reset migration', () => {
  it('clears only ingestion jobs and tournament-owned data', () => {
    expect(sql).toContain('delete from public.tournament_ingestion_jobs where id is not null');
    expect(sql).toContain('delete from public.tournaments where id is not null');
