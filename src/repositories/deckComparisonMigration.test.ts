...
describe('deck comparison migration', () => {
  ...
  it('reads newly normalized cards through canonical card details', () => {
    expect(canonicalSql).toContain('from public.tournament_deck_card_details card');
    expect(canonicalSql).not.toContain('from public.tournament_deck_cards card');
  ...
  it('allows partial samples only when at least 25 mainboard cards exist', () => {
    expect(partialSampleSql).toContain("deck.parsing_status in ('complete', 'partial')");
    ...
    expect(partialSampleSql).toContain('from public.tournament_decks_for_comparison deck');
  });

  it('scores similar Decks in one grouped pass with covering indexes', () => {
    expect(performanceSql).toContain('tournament_deck_cards_mainboard_cover_idx');
    expect(performanceSql).toContain('tournament_decks_commander_comparison_idx');
    ...
    expect(performanceSql).not.toContain('where compared.tournament_deck_id = sample.id');
  });

  it('checks comparison eligibility without canonical detail joins', () => {
    const viewDefinition = performanceSql.slice(
      performanceSql.indexOf('create or replace view public.tournament_decks_for_comparison'),
      performanceSql.indexOf('create or replace function public.get_similar_tournament_decks'),
    );
    expect(viewDefinition).toContain('from public.tournament_deck_cards card');
    expect(viewDefinition).not.toContain('from public.tournament_deck_card_details card');
  });
  ...
  it('removes the tournament mainboard card-count requirement', () => {
    ...
    expect(unrestrictedSampleSql).toContain("deck.parsing_status in ('complete', 'partial')");
