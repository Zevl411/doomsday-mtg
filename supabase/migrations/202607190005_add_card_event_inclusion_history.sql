-- Card history already reports Deck inclusion and the number of eligible
-- events. Track how many distinct events contained the card as a separate
-- observational series for recommendation details.
...
returns table (
  ...
  event_count bigint,
  card_event_count bigint,
  inclusion_rate numeric,
  event_inclusion_rate numeric
)
language sql
stable
set search_path = ''
as $$
  ...
  select
    ...
    coalesce(period_cards.events, 0)::bigint,
    case
      when period_totals.total = 0 then 0
      else coalesce(period_cards.decks, 0)::numeric / period_totals.total
    end,
    case
      when period_totals.events = 0 then 0
      else coalesce(period_cards.events, 0)::numeric / period_totals.events
    end
  from period_totals
