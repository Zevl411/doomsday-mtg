...
describe('canonical card migration', () => {
  it('stores card facts once and provider spellings as aliases', () => {
    ...
    expect(sql).toContain('create table if not exists public.canonical_card_aliases');
  ...
  it('links compact tournament rows to canonical cards', () => {
    ...
    expect(sql).toContain('tournament_deck_cards_canonical_unique_idx');
    expect(sql).toContain('tournament_deck_id, board, canonical_card_id');
  ...
  it('backfills legacy rows and centralizes compatibility reads', () => {
    ...
    expect(sql).toContain('create or replace view public.tournament_deck_card_details');
  ...
  it('moves inclusion and comparison RPCs onto canonical details', () => {
    ...
    expect(sql.match(/from public\.tournament_deck_card_details/g)).toHaveLength(3);
  });
