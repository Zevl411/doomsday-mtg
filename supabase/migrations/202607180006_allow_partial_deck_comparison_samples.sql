-- A partially normalized Deck can still provide a useful descriptive sample.
-- Require at least 25 saved mainboard cards so metadata-only or nearly empty
-- rows never influence comparison statistics.
...
as $$
  ...
  select cards.normalized_card_key, cards.oracle_id, cards.card_name,
    cards.type_line, cards.color_identity, cards.mana_value, cards.deck_count,
    totals.total,
    case when totals.total = 0 then 0
      else cards.deck_count::numeric / totals.total end,
    cards.average_quantity, cards.top16_deck_count,
...
as $$
  ...
  deck_cards as (
    select distinct card.tournament_deck_id,
      case when card.oracle_id is not null
        then 'oracle:' || lower(card.oracle_id::text)
        else 'name:' || lower(trim(card.normalized_card_key))
      end identity_key
