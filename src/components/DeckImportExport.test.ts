...
describe('DeckImportExport', () => {
  ...
  it('disables repeated submissions while loading', async () => {
    vi.mocked(prepareDeckImport).mockImplementation(() => new Promise(() => undefined));
    const wrapper = mountComponent();
    ...
    expect(findButton(wrapper, 'Process Import')?.attributes('disabled')).toBeDefined();
    wrapper.unmount();
  });

  it('aborts an active import when the component unmounts', async () => {
    let receivedSignal: AbortSignal | undefined;
    vi.mocked(prepareDeckImport).mockImplementation((_text, _deck, signal) => {
