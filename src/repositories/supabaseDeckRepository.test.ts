...
function createClient(records: unknown[] = [], error: Error | null = null) {
  ...
  const selectEq = vi.fn((field: string) =>
    field === 'user_id' ? Promise.resolve(result) : { eq: vi.fn(() => ({ maybeSingle })) },
  );
...
describe('supabaseDeckRepository', () => {
  ...
  it('loads only valid records owned by the active user', async () => {
    ...
    await expect(createSupabaseDeckRepository(client, 'user-a').loadDecks()).resolves.toEqual([
      deck,
    ]);
  });
  ...
  it('falls back to legacy deck columns while sharing migration is pending', async () => {
    ...
    expect(upsert.mock.calls[1]?.[0]).toEqual(expect.objectContaining({ deck_data: deck }));
  });
  ...
  it('loads one valid user-owned deck by stable ID', async () => {
    ...
    await expect(createSupabaseDeckRepository(client, 'user-a').loadDeck(deck.id)).resolves.toEqual(
      deck,
    );
  });
  ...
  it('maps provider failures to readable errors', async () => {
    ...
    await expect(createSupabaseDeckRepository(client, 'user-a').loadDecks()).rejects.toThrow(
      'Cloud deck load failed.',
    );
  });
});
