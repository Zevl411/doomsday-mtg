...
import { buildExcludedTitleKeywords, evaluateTournamentTitle } from './tournamentRelevance.ts';
...
describe('tournament relevance', () => {
  ...
  it('excludes Commander Brackets 1 through 4, including compact provider tags', () => {
    ...
    expect(evaluateTournamentTitle('Commander Bracket 2 League').included).toBe(false);
    expect(evaluateTournamentTitle('Bracket 1 Commander Night').included).toBe(false);
    expect(evaluateTournamentTitle('[CoBr4] Optimized Commander').included).toBe(false);
    expect(evaluateTournamentTitle('Commander Bracket 5 Championship').included).toBe(true);
  });
  ...
  it('normalizes translated casual-event signals and accents', () => {
    ...
    expect(evaluateTournamentTitle('Torneo para Principiantes').included).toBe(false);
    expect(evaluateTournamentTitle('Préconstruit Débutant').included).toBe(false);
    expect(evaluateTournamentTitle('Baixa Potência Commander').included).toBe(false);
  });
  ...
  it('adds configured regional terms without removing defaults', () => {
    const keywords = buildExcludedTitleKeywords(['regional phrase, tienda social', undefined]);
