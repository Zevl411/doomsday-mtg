...
export function compareDeckToAggregate(
  ...
  const deckKeys = new Set(
    cards.filter((card) => card.board === 'mainboard').map((card) => card.normalizedCardKey),
  );
  ...
  return {
    ...
    missingCoreCards: aggregate.filter(
      (card) =>
        getInclusionTier(card.inclusionRate) === 'Core' && !deckKeys.has(card.normalizedCardKey),
    ),
    rareCards: cards.filter(
      (card) =>
        card.board === 'mainboard' &&
        getInclusionTier(
          aggregate.find((item) => item.normalizedCardKey === card.normalizedCardKey)
            ?.inclusionRate ?? 0,
        ) === 'Rare',
    ),
  };
