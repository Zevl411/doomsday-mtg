  ...
  <v-container class="pa-0" fluid>
    <div class="d-flex flex-wrap align-center justify-space-between ga-4 mb-6">
      ...
      <div class="d-flex flex-wrap ga-2">
        ...
        <v-btn v-else color="primary" size="large" @click="openCreateDialog"> New deck </v-btn>
      </div>
    </div>
    ...
    <v-row v-if="deckStore.library.decks.length">
      <v-col v-for="deck in decksByRecentUpdate" :key="deck.id" cols="12" md="6" lg="4">
        <DeckLibraryCard
    ...
    <v-card v-else border class="pa-8 text-center" color="surface" rounded="lg">
      ...
      <v-card-text> Create your first deck to start building. </v-card-text>
      <v-btn color="primary" @click="openCreateDialog"> Create a deck </v-btn>
    </v-card>
    ...
    <v-dialog v-model="showBulkDeleteDialog" max-width="480">
      <v-card color="surface" rounded="lg">
        <v-card-title class="px-5 pt-5"> Delete {{ selectedDeckIds.size }} decks? </v-card-title>
        <v-card-text class="px-5">
          This permanently removes the selected decks
          {{ auth.isSignedIn ? 'from your account' : 'from this browser' }}.
          <v-list class="bulk-delete-list mt-4" density="comfortable" lines="one">
            <v-list-item v-for="deck in selectedDecks" :key="deck.id" :title="deck.name">
              <template #prepend>
        ...
        <v-card-actions class="px-5 pb-5">
          ...
          <v-btn color="error" variant="flat" @click="confirmBulkDelete"> Delete decks </v-btn>
        </v-card-actions>
      </v-card>
    </v-dialog>
      ...
      <v-card color="surface" rounded="lg">
        ...
        <v-card-text class="px-5">
          ...
          <p class="text-caption text-medium-emphasis">
            This setting will replace the current visibility of every selected deck.
          </p>
        </v-card-text>
        <v-card-actions class="px-5 pb-5">
          <v-spacer />
          <v-btn variant="text" @click="showBulkVisibilityDialog = false"> Cancel </v-btn>
...
const deletingDeck = computed(() =>
  deckStore.library.decks.find((deck) => deck.id === deletingDeckId.value),
);
const selectedDecks = computed(() =>
  decksByRecentUpdate.value.filter((deck) => selectedDeckIds.value.has(deck.id)),
);
...
function openBulkVisibilityDialog() {
  ...
  const allMatch = selectedDecks.value.every((deck) => deck.visibility === selectedVisibility);
  bulkVisibility.value = allMatch && selectedVisibility ? selectedVisibility : 'public';
  showBulkVisibilityDialog.value = true;
}

function confirmBulkVisibility() {
  deckStore.updateDeckVisibilities([...selectedDeckIds.value], bulkVisibility.value);
