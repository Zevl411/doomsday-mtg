...
export function getAnalyticalCardIdentity(input: {
  ...
  return input.oracleId
    ...
    : `name:${normalizeAnalyticalName(input.normalizedCardKey || input.cardName)}`;
}
...
export function getComparisonCommander(deck: Deck) {
  const names = [deck.commander?.name, deck.partnerCommander?.name].filter((name): name is string =>
    Boolean(name),
  );
...
export function getSampleStatus(count: number): DeckComparisonSampleStatus {
...
export function buildDeckComparisonSummary(
  ...
  const sharedCardCount = [...aggregateIdentityKeys].filter((key) => userCards.has(key)).length;
  ...
  return {
    ...
    sharedCardCount,
    aggregateOverlapRate: aggregateUniqueCards === 0 ? 0 : sharedCardCount / aggregateUniqueCards,
