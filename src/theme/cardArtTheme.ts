...
import { oracleDarkColors, oracleDarkVariables } from './oracleTheme';
 ...
 */
export async function createCardArtTheme(card: ScryfallCard): Promise<CardArtAppTheme> {
  const artUrl = getCardArt(card);
  if (!artUrl) throw new Error('This card does not have artwork to sample.');
  const paletteSourceUrl = buildCardArtProxyUrl(artUrl, import.meta.env.VITE_SUPABASE_URL);
 ...
 */
export function buildCardArtProxyUrl(artUrl: string, supabaseUrl: string | undefined): string {
  if (!supabaseUrl) {
    throw new Error('Card artwork themes require the Supabase card-art proxy to be configured.');
  }
...
export function buildCardThemePalette(colors: RgbColor[]): CardThemePalette {
  ...
  const secondarySource =
    chooseDistinctColor(ranked.slice(1), [primarySource]) ?? rotateChannels(primarySource);
  const accentSource =
  ...
  return {
    ...
    surfaceBright: toHex(mix(darkBase, secondary, 0.2)),
    surfaceLight: toHex(mix(darkBase, accent, 0.28)),
    primary: toHex(primary),
    primaryDarken: toHex(mix(primary, darkBase, 0.3)),
...
export function getVuetifyTheme(preference: CardArtAppTheme) {
 ...
 */
function clusterColors(colors: WeightedColor[], maximumClusters = 5): WeightedColor[] {
  ...
  while (seeds.length < maximumClusters) {
    const candidate = [...colors]
      ...
      .sort((left, right) => clusterSeedScore(right, seeds) - clusterSeedScore(left, seeds))[0];
    if (!candidate) break;
...
function clusterSeedScore(color: WeightedColor, seeds: WeightedColor[]): number {
  const minimumDistance = Math.min(...seeds.map((seed) => colorDistance(color, seed)));
  const channels = [color.red, color.green, color.blue];
  const saturation = (Math.max(...channels) - Math.min(...channels)) / 255;
  return minimumDistance * Math.sqrt(color.count) * (0.55 + saturation);
}

function nearestColorIndex(color: RgbColor, candidates: RgbColor[]): number {
  let nearestIndex = 0;
...
function chooseDistinctColor(
  ...
  return [...candidates].sort(
    (left, right) => distinctColorScore(right, selected) - distinctColorScore(left, selected),
  )[0];
}

function distinctColorScore(color: WeightedColor, selected: RgbColor[]): number {
