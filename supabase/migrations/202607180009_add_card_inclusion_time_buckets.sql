-- Generalize the weekly history added in the previous migration while keeping
-- bucket selection constrained to four supported date_trunc values.
create or replace function public.get_commander_card_inclusion_over_time(
...
as $$
  ...
  period_totals as (
    select eligible.period_start, count(*)::bigint as total
    from eligible
