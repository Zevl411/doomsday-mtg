...
describe('CardSearch', () => {
  ...
  it('waits for the debounce and then searches', async () => {
    ...
    expect(searchCards).toHaveBeenCalledWith('test', expect.any(AbortSignal));
  ...
  it('can request one search result per unique artwork', async () => {
    ...
    expect(searchCards).toHaveBeenCalledWith('sol ring', expect.any(AbortSignal), 'art');
    wrapper.unmount();
  });
  ...
  it('renders multiple selected cards as removable input chips', async () => {
    ...
    expect(wrapper.find('input').attributes('placeholder')).toBeUndefined();
    await wrapper.findAll('.selected-card-chip__close')[1].trigger('click');
  ...
  it('falls back to cached canonical cards during a Scryfall outage', async () => {
    ...
    expect(wrapper.text()).toContain('Scryfall is unavailable. Showing cached tournament cards.');
    expect(wrapper.text()).toContain('Test Card');
    wrapper.unmount();
  });
