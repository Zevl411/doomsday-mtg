<template>
  <v-container class="pa-0" fluid>
    <div class="home-hero mb-10">
      <div class="home-hero-brand d-flex align-center justify-center pa-4 pa-md-8">
        <AnimatedOracleLogo
      ...
      <div class="home-hero-content">
        ...
        >
          ...
          <h1 class="text-h3 font-weight-bold mb-3">Build with better information</h1>
          <p class="text-body-1 text-medium-emphasis mb-6">
            Turn competitive Commander data into practical deck-building decisions.
          </p>
          ...
          <v-btn class="mt-4 align-self-start" color="primary" @click="startNewDeck">
            Start building
          </v-btn>
        </v-card>
      </div>
    ...
    <v-row class="mb-2" align="center">
      ...
      <v-col class="text-right" cols="auto">
        <v-btn :to="{ name: 'deck-library' }" variant="text"> View all decks </v-btn>
      </v-col>
    </v-row>

    <v-row v-if="recentDecks.length" class="mb-8">
      <v-col v-for="deck in recentDecks" :key="deck.id" cols="12" sm="6" lg="3">
        <DeckLibraryCard
    ...
    <v-card v-else border class="mb-8 pa-8 text-center" color="surface" rounded="lg">
      <v-card-title>No decks yet</v-card-title>
      <v-card-text>Create a deck to begin building.</v-card-text>
      <v-card-actions class="justify-center">
        <v-btn color="primary" @click="startNewDeck"> Create a deck </v-btn>
      </v-card-actions>
    </v-card>
    ...
    <v-row class="mb-2" align="center">
      ...
      <v-col class="d-flex ga-2 justify-end" cols="auto">
        ...
        <v-btn :to="{ name: 'tournaments' }" variant="text"> Tournaments </v-btn>
      </v-col>
    </v-row>
    ...
    <AppLoadingSkeleton
      ...
      variant="table"
    />
    <v-alert v-else-if="tournamentError" class="mb-6" type="info" variant="tonal">
      {{ tournamentError }}
    </v-alert>
    <v-row v-else>
      <v-col cols="12" lg="7">
          ...
          <v-list v-if="commanderStats.length">
            ...
            >
              <v-list-item-title class="d-flex align-center ga-2">
                ...
                <ColorIdentitySymbols :colors="commander.colorIdentity" size="small" />
              </v-list-item-title>
              <v-list-item-subtitle>
                {{ commander.entries }} entries · {{ percent(commander.metaShare) }} of the field
              </v-list-item-subtitle>
              <template #append>
  ...
  <v-dialog v-model="showNameDialog" max-width="480">
    <v-card color="surface" rounded="lg">
      ...
      <v-card-actions class="px-5 pb-5">
        ...
        <v-btn color="primary" variant="flat" @click="submitName"> Save </v-btn>
      </v-card-actions>
    </v-card>
  </v-dialog>
  ...
  <v-dialog v-model="showDeleteDialog" max-width="480">
    <v-card color="surface" rounded="lg">
      ...
      <v-card-text class="px-5"> This permanently removes “{{ deletingDeck?.name }}”. </v-card-text>
      <v-card-actions class="px-5 pb-5">
        <v-spacer />
...
import type { CommanderMetagameStats, Tournament } from '../models/tournament';
...
onMounted(async () => {
  try {
    ...
    tournamentError.value = 'Tournament data is not available yet. Your decks are still ready.';
  } finally {
