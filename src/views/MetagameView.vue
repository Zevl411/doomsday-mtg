<template>
  <v-container class="pa-0" fluid>
    <div class="d-flex flex-wrap align-start justify-space-between ga-3 mb-6">
      <div>
        ...
        <p class="text-medium-emphasis">
          Tournament results from TopDeck and EDHTop16. Percentages include their sample sizes and
      ...
      <AppMobileFilterPanel>
        <v-row>
          <v-col cols="12" sm="6" md="2">
            <v-select v-model="timeframe" label="Timeframe" :items="timeframeOptions" />
          </v-col>
          <v-col cols="12" sm="6" md="2">
            <v-text-field
              v-model="startDate"
    ...
    <v-row v-else class="commander-grid">
      <v-col v-for="item in stats" :key="item.commanderKey" cols="12" sm="6" md="6" lg="4" xl="3">
        <v-card
          border
        ...
        >
          <div v-if="item.imageUrls?.length" class="commander-card__images">
            <v-img
          ...
          <div v-else class="commander-card__placeholder d-flex align-center justify-center">
            No card image
          </div>

          <v-card-title class="commander-card__title">
            <span>{{ item.commanderName }}</span>
            <ColorIdentitySymbols :colors="item.colorIdentity" size="small" />
          </v-card-title>
          <v-card-text>
...
type MetagameTimeframe =
  'week' | 'month' | 'three-months' | 'six-months' | 'year' | 'all' | 'custom';
...
async function load() {
  ...
  try {
    ...
    errorMessage.value = error instanceof Error ? error.message : 'Unable to load metagame data.';
  } finally {
    loading.value = false;
  }
}
...
.commander-card:hover {
  ...
  box-shadow: 0 0 14px rgb(var(--v-theme-primary), 0.2);
