...
function deckCard(key: string): NormalizedTournamentDeckCard {
  return {
    id: key, board: 'mainboard', normalizedCardKey: key, cardName: key,
    quantity: 1, colorIdentity: [], colors: [], isBasicLand: false,
  }
}

function inclusion(key: string, rate: number): CommanderCardInclusion {
  return {
    normalizedCardKey: key, cardName: key, colorIdentity: [], deckCount: 1,
    totalEligibleDecks: 1, inclusionRate: rate, averageQuantity: 1,
    top16DeckCount: 1, top16InclusionRate: rate,
    firstPlaceDeckCount: 0, firstPlaceInclusionRate: 0,
  }
}
