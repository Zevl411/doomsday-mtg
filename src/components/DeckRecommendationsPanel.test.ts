...
const { getCardInclusionOverTime, getCardsByExactNames, getSuggestionEvidence } = vi.hoisted(
  () => ({
...
function card(id: string, oracleId: string, name: string): ScryfallCard {
  return {
    id,
    oracle_id: oracleId,
    name,
...
beforeEach(() => {
  ...
  getCardsByExactNames.mockResolvedValue([card('suggested', suggestionOracleId, 'Suggested Card')]);
...
describe('DeckRecommendationsPanel', () => {
  it('closes recommendation details from the header button', async () => {
    ...
    deckStore.setCommander(
      card('commander', '33333333-3333-4333-8333-333333333333', 'Test Commander'),
    );
    ...
    expect(controls.get('.recommendation-card__metrics').attributes('aria-label')).toBe(
      '1 supporting card; 25 Deck sample',
    );
    expect(controls.find('[aria-label="Add Suggested Card to Mainboard"]').exists()).toBe(true);
  ...
  it('loads an external Deck without exposing add actions in read-only mode', async () => {
    ...
    expect(wrapper.find('[aria-label="Add Suggested Card to Mainboard"]').exists()).toBe(false);
