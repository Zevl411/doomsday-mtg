...
describe('tournament ingestion function boundaries', () => {
  ...
  it('uses stable upsert identities and supports dry runs', () => {
    ...
    expect(functionSource).toContain("onConflict: 'tournament_id,source_entry_key'");
    expect(functionSource).toContain('if (options.dryRun) return');
  });

  it('does not retain redundant raw tournament payloads', () => {
    expect(functionSource).not.toContain('source_payload: tournament.raw');
  ...
  it('bounds concurrent tournament processing', () => {
    expect(functionSource).toContain('mapWithConcurrency(tournaments, 3');
  });

  it('allows the headers sent by the Supabase browser client', () => {
    expect(functionSource).toContain('authorization, x-client-info, apikey, content-type');
  });

  it('guards the development reset with an exact confirmation token', () => {
    expect(functionSource).toContain("'clear-tournament-data'");
    expect(functionSource).toContain("request.confirmationToken !== 'CLEAR TOURNAMENT DATA'");
    expect(functionSource).toContain("'clear_tournament_ingestion_data'");
  });

  it('preserves useful PostgREST error messages', () => {
    expect(functionSource).toContain("typeof error.message === 'string'");
    expect(functionSource).toContain("typeof error.code === 'string'");
  });
});

describe('browser ingestion orchestration', () => {
  it('splits direct date discovery into bounded provider windows', () => {
    expect(repositorySource).toContain('createDateWindows(options.startDate, options.endDate, 3)');
    expect(repositorySource).toContain('Tournament metadata ingestion failed:');
  });
});
...
describe('normalized tournament deck ingestion boundaries', () => {
  it('persists resolved Commander color identity on tournament entries', () => {
    expect(deckIngestionFunctionSource).toContain("item.candidate.board === 'commander'");
  ...
  it('filters missing normalized Decks before applying the batch limit', () => {
    expect(deckIngestionFunctionSource).toContain('const ENTRY_BATCH_SIZE = 25');
    expect(deckIngestionFunctionSource).toContain("'get_tournament_deck_ingestion_candidates'");
  ...
  it('bulk-classifies unavailable Decks before the bounded entry batch', () => {
    expect(deckIngestionFunctionSource).toContain("'mark_unavailable_tournament_decks'");
    expect(deckIngestionFunctionSource).toContain('report.decksUnavailable += skipped');
    expect(deckIngestionFunctionSource.indexOf("'mark_unavailable_tournament_decks'")).toBeLessThan(
      deckIngestionFunctionSource.indexOf("'get_tournament_deck_ingestion_candidates'"),
    );
  });

  it('reuses persistent canonical aliases before calling Scryfall', () => {
    expect(deckIngestionFunctionSource).toContain("from('canonical_card_aliases')");
    expect(deckIngestionFunctionSource).toContain('loadCanonicalCardCache');
    expect(deckIngestionFunctionSource).toContain("from('canonical_cards')");
    expect(deckIngestionFunctionSource).toContain('canonical_card_id: canonicalCardId');
    expect(deckIngestionFunctionSource).toContain(
  ...
  it('accepts double-faced Scryfall cards without top-level colors', () => {
    expect(deckIngestionFunctionSource).toContain('value.colors === undefined');
    expect(deckIngestionFunctionSource).toContain(
      'card.card_faces?.flatMap((face) => face.colors ?? [])',
    );
    expect(deckIngestionFunctionSource).toContain('colors: getResolvedColors(card)');
  });

  it('releases redundant provider payloads after complete normalization', () => {
    expect(deckIngestionFunctionSource).toContain('.update({ source_payload: null })');
    expect(deckIngestionFunctionSource).toContain("completeness.status === 'complete'");
  });

  it('continues metadata batches through bounded Deck normalization', () => {
    expect(workerFunctionSource).toContain("stage: 'decks'");
    expect(workerFunctionSource).toContain('/functions/v1/ingest-tournament-decks');
    expect(workerFunctionSource).toContain('onlyMissing: true');
    expect(workerFunctionSource).toContain("status: hasMore ? 'pending' : 'completed'");
  });

  it('lets direct metadata ingestion enqueue only the Deck stage', () => {
    expect(functionSource).toContain("'create-deck-job'");
    expect(functionSource).toMatch(/function isJobAction[\s\S]*'create-deck-job'/);
...
describe('tournament decklist function boundaries', () => {
  ...
  it('loads one provider entry on demand and returns preview image data', () => {
    expect(decklistFunctionSource).toContain('query DoomsdayTournamentEntryDeck');
  ...
  it('parses stored TopDeck Decks on demand without normalized Deck rows', () => {
    expect(decklistFunctionSource).toContain("tournament?.source === 'topdeck'");
    ...
    expect(decklistFunctionSource).toContain(".eq('id', tournamentEntryId)");
  });
  ...
  it('refetches a targeted TopDeck event when its stored payload was released', () => {
    ...
    expect(decklistFunctionSource).toContain('tournamentIds: [tournamentId]');
    expect(decklistFunctionSource).toContain("Deno.env.get('TOPDECK_API_KEY')");
  });

  it('validates entry IDs and exposes browser CORS headers', () => {
    expect(decklistFunctionSource).toContain('.test(tournamentEntryId)');
    expect(decklistFunctionSource).toContain('authorization, x-client-info, apikey, content-type');
  });
});
