...
export const dataHealthRepository = {
    ...
    const [summaryResult, readinessResult, unresolvedResult, jobsResult, regionsResult] =
      await Promise.all([
        ...
        supabase!.rpc('get_commander_analytics_readiness', {
          p_minimum_complete_decks: Math.max(0, filters.minimumCompleteDecks ?? 0),
        ...
        supabase!.rpc('get_unresolved_card_health', {
          p_minimum_occurrences: Math.max(1, filters.minimumOccurrences ?? 1),
          p_provider: filters.provider ?? null,
  ...
  async runSmokeTest(
    ...
    const [inclusionResult, aggregateResult, similarityResult] = await Promise.all([
    ...
    checks.push({
      ...
      status: similarities.every((row) => Boolean(row.tournamentDeckId)) ? 'pass' : 'fail',
      message: `${similarities.length} returned Deck IDs resolved through the similarity RPC.`,
    });
    checks.push({
      label: 'Canonical card references',
      status: inclusion.every((row) => Boolean(row.normalizedCardKey)) ? 'pass' : 'fail',
...
export function parseDataHealthSummary(value: unknown): DataHealthSummary {
  ...
  return {
    ...
    tournamentCardWithoutCanonicalCount: row.tournament_card_without_canonical_count,
    ...
    commanderWithTwentyCompleteCount: row.commander_with_twenty_complete_count,
    ...
    lastSuccessfulTournamentIngestion: optionalString(row.last_successful_tournament_ingestion),
    lastSuccessfulDeckNormalization: optionalString(row.last_successful_deck_normalization),
  };
}

export function parseCommanderReadinessRows(value: unknown): CommanderReadiness[] {
  ...
  return value.map((row) => {
    ...
    return {
      ...
      oneSidedExtractionFailureCount: row.one_sided_extraction_failure_count,
    };
  });
}

export function parseUnresolvedCardRows(value: unknown): UnresolvedCardHealth[] {
  if (!Array.isArray(value)) {
...
export function parseIngestionJobHealthRows(value: unknown): IngestionJobHealth[] {
  if (!Array.isArray(value)) {
...
const SUMMARY_NUMBER_FIELDS = [
  'tournament_count',
  'entry_count',
  'topdeck_tournament_count',
  'edhtop16_tournament_count',
  'topdeck_entry_count',
  'edhtop16_entry_count',
  'tournament_with_location_count',
  'tournament_without_location_count',
  'tournament_missing_date_count',
  'excluded_casual_event_count',
  'structured_entry_count',
  'plaintext_entry_count',
  'url_only_entry_count',
  'missing_decklist_entry_count',
  'normalized_deck_count',
  'complete_deck_count',
  'partial_deck_count',
  'unavailable_deck_count',
  'invalid_deck_count',
  'canonical_card_count',
  'canonical_alias_count',
  'canonical_with_oracle_count',
  'fallback_identity_count',
  'unresolved_card_row_count',
  'tournament_card_count',
  'tournament_card_without_canonical_count',
  'suspicious_alias_count',
  'commander_with_one_complete_count',
  'commander_with_five_complete_count',
  'commander_with_twenty_complete_count',
  'commander_with_fifty_complete_count',
  'commander_without_complete_count',
  'paired_commander_sample_count',
...
const READINESS_NUMBER_FIELDS = [
  ...
  'state_region_count',
  'top16_sample_count',
  'first_place_sample_count',
  'regional_sample_count',
  'unresolved_card_rate',
...
function requireFields<Fields extends readonly string[]>(
...
function isSampleStatus(value: unknown): value is CommanderReadiness['sampleStatus'] {
  return ['unavailable', 'insufficient', 'limited', 'sufficient'].includes(String(value));
}
