...
describe('deck comparison service', () => {
  ...
  it('deduplicates mainboard identities while retaining quantities', () => {
    ...
    deck.cards = [
      {
        card: { id: 'a', oracle_id: 'same', name: 'One', type_line: '', color_identity: [] },
        quantity: 1,
      },
      {
        card: { id: 'b', oracle_id: 'same', name: 'Two', type_line: '', color_identity: [] },
        quantity: 2,
      },
    ];
  ...
  it('excludes Commander and auxiliary boards from user comparison', () => {
    ...
    deck.sideboard = [
      {
        card: { id: 's', oracle_id: 'side', name: 'Side', type_line: '', color_identity: [] },
        quantity: 1,
      },
    ];
  ...
  it('assigns descriptive categories and calculates aggregate overlap', () => {
    ...
    deck.cards = [
      {
        card: { id: '1', oracle_id: 'core', name: 'Core', type_line: '', color_identity: [] },
        quantity: 1,
      },
      {
        card: { id: '2', oracle_id: 'rare', name: 'Rare', type_line: '', color_identity: [] },
        quantity: 1,
      },
      { card: { id: '3', name: 'Unresolved', type_line: '', color_identity: [] }, quantity: 1 },
    ];
    const summary = buildDeckComparisonSummary(
      deck,
      [
  ...
  it('uses the same unordered pair key as tournament Commander identity', () => {
    ...
    expect(getComparisonCommander(deck).key).toBe(normalizeCommanderIdentity('Kraum // Tymna').key);
  });
});
