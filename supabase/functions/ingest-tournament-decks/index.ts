...
Deno.serve(async (request) => {
  ...
  try {
    ...
    const isWorker = Boolean(
      workerSecret && request.headers.get('x-ingestion-worker-secret') === workerSecret,
    );
    if (!isWorker) {
    ...
    if (options.endDate) {
      tournamentQuery = tournamentQuery.lt('event_date', addUtcDays(options.endDate, 1));
    }
    if (options.tournamentIds?.length) {
      tournamentQuery = tournamentQuery.in('source_tournament_id', options.tournamentIds);
    }
    ...
    if (options.onlyMissing && !options.dryRun) {
      const { data: unavailableCount, error: unavailableError } = await admin.rpc(
      ...
      if (unavailableError) {
        throw databaseError('classify unavailable tournament Decks', unavailableError);
      }
    ...
    let query = admin
      ...
      .select('id, tournament_id, source_entry_id, commander_key, commander_name, source_payload')
    ...
    for (const entry of entries ?? []) {
      ...
      if (!options.dryRun && !options.retryPartial && oldDeck?.parsing_status === 'partial') {
        report.decksUnchanged += 1;
        continue;
      }
      ...
      const hasPlaintext =
        typeof decklist === 'string' && decklist.trim() !== '' && !/^https?:\/\//i.test(decklist);
      ...
      if (candidates.length) {
        await loadCanonicalCardCache(admin, candidates, resolutionCache, canonicalIdCache);
      }
      ...
      const completeness = evaluateDeckCompleteness(candidates, unresolved.length, issues);
          ...
          resolved
            .filter(
              (item) =>
                ...
                commanderNames.has(normalizeCardKey(item.card?.name ?? item.candidate.name)),
            )
      ...
      const { error: deleteError } = await admin
        ...
        .delete()
        .eq('tournament_deck_id', storedDeck.id);
      if (deleteError) {
        throw databaseError('replace tournament Deck cards', deleteError);
      }
      const cardRows = resolved.flatMap(({ candidate, card }) => {
        const canonicalCardId = canonicalIdCache.get(normalizeCardKey(candidate.name));
      ...
      if (cardRows.length) {
        const { error: cardsError } = await admin.from('tournament_deck_cards').upsert(cardRows, {
...
async function loadCanonicalCardCache(
  ...
  const keys = [...new Set(candidates.map((card) => normalizeCardKey(card.name)))].filter(
    (key) => !canonicalIdCache.has(key),
  );
...
async function saveCanonicalCards(
  ...
  const newCards = resolved.filter(
    (item): item is { candidate: CardCandidate; card: ResolvedCard } =>
      Boolean(item.card) && !canonicalIdCache.has(normalizeCardKey(item.candidate.name)),
  );
  ...
  const { data: stored, error: storedError } = await admin
    .from('canonical_cards')
    .select('*')
    .in('normalized_card_key', canonicalNames);
  if (storedError) throw databaseError('reload canonical cards', storedError);
  const canonicalByName = new Map(
    (stored ?? []).map((card: CanonicalCardRow) => [card.normalized_card_key, card]),
  );
  ...
  for (const { candidate, card } of newCards) {
    ...
    for (const key of new Set([normalizeCardKey(candidate.name), ...getLookupNames(card)])) {
...
async function resolveCandidates(
  ...
  const missing = [...new Set(candidates.map((card) => card.name))].filter(
    (name) => !cache.has(normalizeCardKey(name)),
  );
  for (let index = 0; index < missing.length; index += BATCH_SIZE) {
    ...
    const cards =
      isRecord(body) && Array.isArray(body.data) ? body.data.filter(isResolvedCard) : [];
    for (const name of names) {
      const key = normalizeCardKey(name);
      const frontFaceKey = normalizeCardKey(name.split('//')[0]?.trim() ?? name);
    ...
    for (const name of names.filter((item) => cache.get(normalizeCardKey(item)) === null)) {
      ...
      cache.set(normalizeCardKey(name), isResolvedCard(card) ? card : null);
    }
  }
...
function validateRequest(value: unknown): DeckIngestionRequest {
  ...
  return {
    ...
    provider:
      value.provider === 'topdeck' || value.provider === 'edhtop16' ? value.provider : undefined,
...
function isResolvedCard(value: unknown): value is ResolvedCard {
  return (
    ...
    (value.colors === undefined ||
      (Array.isArray(value.colors) && value.colors.every((color) => typeof color === 'string'))) &&
    (value.card_faces === undefined ||
      (Array.isArray(value.card_faces) &&
        value.card_faces.every(
          (face) =>
            isRecord(face) &&
