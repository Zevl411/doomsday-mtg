-- Suggestions reuse the v0.4 support, confidence, lift, and complete-Deck
-- boundaries. This batched RPC returns evidence only; it does not infer Deck
-- strategy, card quality, expected performance, additions, or removals.
...
as $$
  ...
  -- A personal Deck's existing cards are evidence sources, never candidates.
  where not (
    suggested.oracle_id = any(
      coalesce(p_source_oracle_ids, '{}'::uuid[])
    )
  )
