...
describe('association suggestion repository validation', () => {
  ...
  it('deduplicates sources and passes the existing association filters', async () => {
    ...
    expect(mocks.rpc).toHaveBeenCalledWith('get_association_based_card_suggestions', {
  ...
  it('does not call Supabase without Oracle-backed source cards', async () => {
    await expect(
      associationSuggestionRepository.getSuggestionEvidence('kinnan', ['legacy-name']),
    ).resolves.toEqual([]);
  ...
  it('maps database errors to stable non-strategic copy', async () => {
    ...
    await expect(
      associationSuggestionRepository.getSuggestionEvidence('kinnan', [sourceId]),
