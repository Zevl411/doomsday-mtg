...
describe('DeckComparisonView', () => {
  ...
  it('compares a Commander deck with an empty mainboard', async () => {
    ...
    expect(mocks.compare).toHaveBeenCalledWith(expect.any(String), [], expect.any(Object));
    expect(wrapper.text()).not.toContain('Add at least one mainboard card');
  });
  ...
  it('renders summary categories, sample status, and similar Deck links', async () => {
    ...
    expect(wrapper.find('[aria-label="Cards in Your Deck row 1"]').text()).toBe('1');
    expect(wrapper.find('tbody tr').classes()).toContain('comparison-row--high');
    ...
    expect(mobileStatsPanel.text()).toContain('Aggregate overlap is the share of cards');
    ...
    expect(mobileCard.findComponent({ name: 'VImg' }).props('cover')).toBe(false);
  });
  ...
  it('does not present missing normalized data as a zero-percent result', async () => {
    ...
    expect(wrapper.text()).toContain('No complete tournament card data');
