...
export const associationSuggestionRepository = {
  async getSuggestionEvidence(
    ...
    const uniqueSourceIds = [...new Set(sourceOracleIds.filter((value) => isUuid(value)))];
    if (!uniqueSourceIds.length) return [];

    const { data, error } = await supabase.rpc('get_association_based_card_suggestions', {
      ...
      p_minimum_supporting_cards: positiveInteger(filters.minimumSupportingCards, 2),
...
export function parseAssociationSuggestionRows(value: unknown): AssociationSuggestionRow[] {
